...
// Notice we added { role = 'admin' } here! If we don't pass a role, it assumes admin.
export default function Sidebar({ role = 'admin' }) {
  const location = useLocation();
  const navigate = useNavigate();

  // 1. DEFINE ALL LINKS AND THEIR PERMISSIONS
  const menuItems = [
    {
      title: 'Dashboard',
      // Dynamic path: Officers go to their dash, Admins go to theirs
      path: role === 'officer' ? '/officer/dashboard' : '/dashboard', 
    ...
    {
      title: 'Complaints',
      // Send officers to their own specific complaints page
      path: role === 'officer' ? '/officer/complaints' : '/complaints', 
      allowedRoles: ['admin', 'officer'],
    ...
    {
      title: 'Settings',
      // Send officers to an officer-specific settings route
  ...
  // 2. FILTER THE MENU BASED ON WHO IS LOGGED IN
    ...
    <aside className="w-64 bg-[#0F172A] text-white h-screen flex flex-col flex-shrink-0">
      ...
      <div className="p-4 border-t border-slate-800">
        <div className="flex items-center">
          <div className="w-10 h-10 rounded-full bg-[#F59E0B] flex items-center justify-center text-white font-bold text-[14px]">
            {role === 'admin' ? 'AU' : 'OF'}
          </div>
          <div className="ml-3">
            <p className="text-[13px] font-bold text-white">
              {role === 'admin' ? 'Admin User' : 'Field Officer'}
            </p>
